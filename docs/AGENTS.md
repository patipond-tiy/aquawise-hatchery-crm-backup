...
| `bmad/` | Execution layer. `README.md`, then the four governing docs in precedence order: `prd.md` (what) → `architecture.md` (rules) → `code-design.md` (how; §16 = merge gate) → `security.md` (threats). **Runnable stories** in `bmad/stories/` (story-only); **QA/audit process artifacts** in `bmad/qa/` (alignment + executability audits, fix punch-lists — NOT stories); QA gates in `bmad/uat/`; consolidated change record in `bmad/CHANGELOG.md`. |
...
- A story's references (migration files, tables, RPCs, RBAC actions, paths) are **acceptance criteria, not suggestions** — grep-verified by `bmad/qa/04.executability-audit.md`. Honor them exactly.
