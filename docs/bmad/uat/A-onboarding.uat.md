...
- Storage bucket `hatchery-logos` exists in the Supabase project (migration `012_storage_logos.sql`)
...
**Then:** The browser redirects to `/th` (the dashboard) with no error; a `hatcheries` row, a `hatchery_members` row with `role = 'owner'`, a `notification_settings` row, and a `scorecard_settings` row are all created atomically via the `create_hatchery` RPC
...
5. In the Supabase dashboard → Table Editor:
   - `hatcheries`: confirm exactly one new row for this user; note `id` as `{hatchery_id}`.
   - `hatchery_members`: confirm one row where `hatchery_id = {hatchery_id}` and `role = 'owner'`.
   - `notification_settings`: confirm one row for `{hatchery_id}`.
   - `scorecard_settings`: confirm one row for `{hatchery_id}`.
...
**Then:** The browser redirects to `/th`; no new `hatcheries` or `hatchery_members` rows are created

**Verification:**

Manual — `USE_MOCK=false`

1. Note the current row counts: `SELECT COUNT(*) FROM hatcheries` and `SELECT COUNT(*) FROM hatchery_members WHERE role = 'owner'`.
...
**Pass/Fail:** PASS if row counts are unchanged and unit test suite is green. FAIL if a second `hatcheries` or `hatchery_members` row exists after the second sign-in, or if any bootstrap test fails.
...
**When:** The `hatcheries` row is inspected immediately after first sign-in
...
**Then:** A `team_invites` row with 7-day expiry and a 64-char hex token is created; the invitee's `hatchery_members` row has `role = 'lab_tech'`; the invite's `accepted_at` is stamped; the invitee appears in the Settings → Team list
...
4. In Supabase → `team_invites`: confirm row exists with `hatchery_id`, `email`, `role = 'lab_tech'`, `token` (64 chars), `expires_at` ~7 days from now, `accepted_at IS NULL`.
...
7. In Supabase → `hatchery_members`: confirm a row for the invitee with `role = 'lab_tech'`.
...
**Then:** The accept-invite route redirects to a URL containing `?invite=expired`; no `hatchery_members` row is created for the invitee
...
4. Confirm no new `hatchery_members` row exists for the invitee email.
...
**When:** Owner navigates to `/th/settings`; uploads the PNG as the hatchery logo; clicks "บันทึก"
**Then:** The file is stored at `{hatchery_id}/logo.png` in the `hatchery-logos` Storage bucket; `hatchery_brand.logo_url` is updated to the public URL of that object; a success toast appears
...
5. In Supabase Storage → `hatchery-logos` bucket: confirm an object at path `{hatchery_id}/logo.png` exists.
6. In Supabase → `hatchery_brand`: confirm `logo_url` is set to a URL that is publicly accessible (open the URL in a browser; confirm the PNG renders).

**Pass/Fail:** PASS if the Storage object exists at the correct path and `logo_url` in `hatchery_brand` points to a publicly reachable PNG URL. FAIL if the object is absent, the path does not match `{hatchery_id}/logo.png`, or `logo_url` is not updated.
...
**When:** Owner changes `display_name_th` to "โรงฟักทดสอบ" and `display_name_en` to "Test Hatchery"; clicks "บันทึก"; then reloads the settings page
...
1. Navigate to `/th/settings`.
2. Clear and re-type the TH display name field: "โรงฟักทดสอบ".
3. Clear and re-type the EN display name field: "Test Hatchery".
4. Click "บันทึก"; confirm success toast.
5. Hard-reload the page (`Ctrl+Shift+R` / `Cmd+Shift+R`).
6. Confirm the TH input shows "โรงฟักทดสอบ" and the EN input shows "Test Hatchery".
7. In Supabase → `hatchery_brand`: confirm `display_name_th = 'โรงฟักทดสอบ'` and `display_name_en = 'Test Hatchery'`.
...
**When:** Owner attempts to upload a `.txt` file as the hatchery logo and clicks "บันทึก"
**Then:** The action returns an error; an error toast appears mentioning an invalid file type; no new object is created in the `hatchery-logos` bucket
...
5. In Supabase Storage → `hatchery-logos`: confirm no new object was uploaded.
