-- LINE × Nursery-CRM integration — Phase 1: identity bind + outbound push.
-- Adds three tables. Strictly additive; no changes to existing schema.
-- Companion: docs/line-integration-strategy.md

-- ============================================================
-- Nursery brand (per-tenant logo + color for co-branded Flex headers)
-- ============================================================

create table public.nursery_brand (
...
create index customer_bind_tokens_nursery_idx on public.customer_bind_tokens(nursery_id, customer_id);
...
create index line_outbound_events_nursery_idx on public.line_outbound_events(nursery_id, created_at desc);
...
alter table public.nursery_brand          enable row level security;
...
-- nursery_brand: scoped by nursery membership
create policy nursery_brand_rw on public.nursery_brand for all
...
-- customer_bind_tokens: nursery staff can mint + read; consumption happens via service-role from the LIFF bind endpoint.
create policy customer_bind_tokens_rw on public.customer_bind_tokens for all
  using (nursery_id in (select public.current_user_nursery_ids()))
  with check (nursery_id in (select public.current_user_nursery_ids()));

-- line_outbound_events:
--   * nursery staff INSERT + SELECT for their nursery
