...
function applyFilter(c: Customer, filter: Filter): boolean {
  ...
  // AC #3: Restock tab = customers due within 14 days.
  if (filter === 'restock' && (c.restockIn == null || c.restockIn > 14))
    return false;
  if (
    filter === 'concern' &&
    c.status !== 'concern' &&
...
export function CustomersView() {
  ...
  const filtered = customers.filter((c) => {
    // AC #2: case-insensitive search across farm, farmEn, name.
    if (
      q &&
      !(c.farm + c.farmEn + c.name)
        .toLowerCase()
  ...
  return (
    <div>
      ...
      >
        {customers.length} ฟาร์ม ·{' '}
        {customers.filter((c) => c.cycleDay).length} กำลังเลี้ยง
      </div>
